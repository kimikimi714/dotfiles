...
- **Header Emojis (Optional / Recommended)**: Emojis defined in the template file configured in `.gitconfig` (`commit.template`) can optionally be used as prefixes (e.g., `:+1:`, `:new:`, `:bug:`), but providing rich context in the body is the primary requirement.
